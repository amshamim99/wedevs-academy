<?php 
    ...
    function wd_ac_insert_adress( $args = []){
        global $wpdb;
        ...
        if(isset($data['id'])){
            ...
            if(! $inserted){
                return new \WP_Error('insert-to-failed', __('insert failed', 'wedevs-academy'));
            }
            return $wpdb->insert_id;
        }
    }

    function wd_ac_get_addreesses( $args = []){
        global $wpdb;
        ...
        $items = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}ac_addresses ORDER BY {$args['orderby']} {$args['order']}
                LIMIT %d , %d",
                $args['offset'],
                $args['number']
            )
